...
Lit directement les variables depuis le fichier .env
"""
import os
import sys
...
# Lire le .env manuellement pour s'assurer d'avoir la bonne URL
def read_env_file():
    env_vars = {}
...
env = read_env_file()
print(f"DATABASE_URL du .env: {env.get('DATABASE_URL', 'NON DÉFINI')[:80]}...")
...
# Pour l'automatisation via pipe
if sys.stdin.isatty():
